...
export default function FiguraCheckBox(props: InputShortProps) {
                        ...
                        <>
                            <div className={`${wrapper ? wrapper : "input-container-inline"}`}>
                                <input
                                    ...
                                    onChange={e => { checkForErrors(true, name, e.target.checked.toString(), "checkbox", context.dispatch, context.formState, context.formID); }}
                                />
                                {children}
                            </div>
...
FiguraCheckBox.displayName = "FiguraCheckBox"; //we do this because children.name is unstable. Therefore we explicitly define a displayName