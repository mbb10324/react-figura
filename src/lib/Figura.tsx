...
import { fallBack, typeMapper, validationMapper } from "./FiguraUtils/ValidationUtils";
...
export default function Figura(props: FiguraProps) {
	...
	const onEvent = useCallback((value: string, name: string, type: string) => {
		dispatch({
			type: "INPUT_UPDATE",
			data: { name: name, value: value, type: type, touched: true, validator: fallBack },
		});
	...
	if (children && Array.isArray(children)) {
		...
		children.forEach((child) => {
			...
			if (child.props.name) {
				...
				fieldNames.push({ name: child.props.name, type: childType, validation: childValidation, formID: formID });
			}
		});
	}
