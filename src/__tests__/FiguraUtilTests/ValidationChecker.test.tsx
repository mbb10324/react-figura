import { validationChecker, validateText, validateEmail, validatePassword, validateConfirmPassword, validatePhone, validateCheck, validateRadio, validateRange, validateSelect, validateTime, validateTimeMilitary, validateTextArea, validateNumber, validateDate, validateDateLocal, validateHidden, validateColor, validateWeek, validateMonth, validateFile, validateUrl } from "../../lib/FiguraUtils/ValidationChecker";

describe("ValidationChecker", () => {
    describe("validationChecker", () => {
        it("should validate text", () => {
            const result = validationChecker("text", "John Doe", null);
            expect(result).toEqual({ hasError: false, error: "" });
        });
        it("should return default values for unknown type", () => {
            const result = validationChecker("unknown", "", null);
            expect(result).toEqual({ hasError: false, error: "" });
        });
    });
...
describe("Validation Functions", () => {
    ...
    test("validateConfirmPassword", () => {
        const formState = {
            FiguraPassword0: { value: "Test123!", type: "password" },
        };
    ...
    test("validateRange", () => {
        ...
        expect(validateRange("")).toEqual({ hasError: true, error: "You must choose a range." });
    });
    ...
    test("validateColor", () => {
        ...
        expect(validateColor("")).toEqual({ hasError: true, error: "Choose a color" });
    });
